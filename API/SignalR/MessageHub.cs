...
public class MessageHub(IMessageRepository messageRepository, IUserRepository userRepository, IMapper mapper, IHubContext<PresenceHub> presenceHub) : Hub
{
    ...
    private async Task<Group> AddToGroup(string groupName)
    {
        try
        {
            ...
            if (await messageRepository.SaveAllAsync())
            {
                Console.WriteLine($"Added {username} to group {groupName}");
                return group;
            }
            else
            {
        ...
        {
            Console.WriteLine($"Error adding to group {groupName}: {ex.Message}");
            throw new HubException($"Failed to add to group {groupName}: {ex.Message}");
        }
    }
    ...
    private async Task<Group> RemoveFromMessageGroup()
    {
        ...
        Console.WriteLine("Group not found, cannot remove from group.");
